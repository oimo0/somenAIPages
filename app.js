...
function renderSidebarUsage(){
 ...
 const v=usageState.items[k],row=document.createElement('div');row.className='usage-row '+k;const label=document.createElement('span');label.textContent=qualityInfo[k].label;const value=document.createElement('b');value.textContent=v.remaining+' / '+v.limit;const bar=document.createElement('progress');bar.max=v.limit||1;bar.value=v.remaining;bar.setAttribute('aria-label',qualityInfo[k].label+'：残り'+v.remaining+'回 / '+v.limit+'回');row.append(label,value,bar);wrap.append(row);
